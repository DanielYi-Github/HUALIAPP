...
let Common = {
	...
	*/
	 ...
	 * @param {Number or String} column 列数或列名称
	 * @param Number batchNum 批量数量
	 * @returns 
	 */
	tranBatchInsertSQL(tableName, datas, dataFun, column, batchNum) {
		//拼好的语句及对应资料数组
		let excuteList = []
		//拼写初始语句
		let insertInitSQL = ""
		...
		switch (typeof (column)) {
			case "number":
				columnLength = column
				insertInitSQL = 'INSERT INTO ' + tableName + ' '
				break
			case "string":
				columnLength = column.split(",").length
				insertInitSQL = 'INSERT INTO ' + tableName + '(' + column + ')'
				break
			default:
				return excuteList
		}
	...
	/**
	 * 将资料转换成SQLiteUtil deleteData、insertData方法的参数格式
	 * @param String tableName 
	 * @param Array datas 
	 * @param Function dataFun 
	 * @param {Number or String} column 
	 * @param Number batchNum 
	 * @returns 
	 */
	tranDiffUpdateSQL(tableName, datas, dataFun, column, batchNum) {
		//拼好的语句及对应资料数组
	...
	/**
	 * 判断text简繁体是否相同,并返回相应的关键字
	 * @param {*} text 
	 * @returns isSEqualsT为false keyword有值,isSEqualsT为true sKeyword、tKeyword有值
	 */
	getSearchKeyword(text) {
		let isSEqualsT = true //简繁体是否相同
		let keyword = ""//关键字
		let sKeyword = ""//繁体关键字
		let tKeyword = ""//简体关键字
		let sText = sify(text)//转换为简体
		let tText = tify(text)//转换为繁体

		if (sText === tText) {//简繁体相同
			keyword = text
		} else {//简繁体不同
			isSEqualsT = false
			sKeyword = sText
	 ...
	 * 
	 * @param Array arr 
	 * @returns 返回过滤重复元素后的数组
	 */
	filterDuplicateElement(arr) {
		var hashTable = {};
