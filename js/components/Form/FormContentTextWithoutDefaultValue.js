...
class FormContentTextWithoutDefaultValue extends Component {
	constructor(props) {
		super(props);
	...
	render() {
		...
		if (editable) {
			...
			return(
				  ...
				  >
		 			<Label style={{flex: 0, color:"#FE1717"}}>{required}</Label>
				  	<Label style={{flex: 1.2}}>{this.state.labelname}</Label>
				    <Input 
						// multiline 
						scrollEnabled = {false}
						value         ={value} 
...
export default connectStyle( 'Component.FormContent', {} )(FormContentTextWithoutDefaultValue);
